...
class StangsScraperTest extends ScraperTestBase {
    @Test
    void extractsDishesFromFixture() {
        server.stubFor(get("/lunch").willReturn(ok()
                .withHeader("Content-Type", "text/html; charset=utf-8")
                .withBodyFile("stangs.html")));

        WeeklyMenu menu = new StangsScraper().scrape(page, server.url("/lunch"));

        assertFalse(menu.isEmpty(), "StangsScraper returned empty menu from fixture");
