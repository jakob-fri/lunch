...
class YogiScraperTest extends ScraperTestBase {
    @Test
    void extractsDishesFromFixture() {
        server.stubFor(get("/lunch").willReturn(ok()
                .withHeader("Content-Type", "text/html; charset=utf-8")
                .withBodyFile("yogi.html")));

        WeeklyMenu menu = new YogiScraper().scrape(page, server.url("/lunch"));

        assertFalse(menu.isEmpty(), "YogiScraper returned empty menu from fixture");
