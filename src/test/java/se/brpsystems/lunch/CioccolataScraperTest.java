...
class CioccolataScraperTest extends ScraperTestBase {
    @Test
    void extractsDishesFromFixture() {
        server.stubFor(get("/lunch").willReturn(ok()
                .withHeader("Content-Type", "text/html; charset=utf-8")
                .withBodyFile("cioccolata.html")));

        WeeklyMenu menu = new CioccolataScraper().scrape(page, server.url("/lunch"));

        assertFalse(menu.isEmpty(), "CioccolataScraper returned empty menu from fixture");
