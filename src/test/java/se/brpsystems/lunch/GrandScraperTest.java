...
class GrandScraperTest extends ScraperTestBase {
    @Test
    void extractsDishesFromFixture() {
        server.stubFor(get("/lunch").willReturn(ok()
                .withHeader("Content-Type", "text/html; charset=utf-8")
                .withBodyFile("grand.html")));

        WeeklyMenu menu = new GrandScraper().scrape(page, server.url("/lunch"));

        assertFalse(menu.isEmpty(), "GrandScraper returned empty menu from fixture");
