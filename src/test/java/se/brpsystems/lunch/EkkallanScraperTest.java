...
class EkkallanScraperTest extends ScraperTestBase {
    @Test
    void extractsDishesFromFixture() {
        server.stubFor(get("/lunch").willReturn(ok()
                .withHeader("Content-Type", "text/html; charset=utf-8")
                .withBodyFile("ekkallan.html")));

        WeeklyMenu menu = new EkkallanScraper().scrape(page, server.url("/lunch"));

        assertFalse(menu.isEmpty(), "EkkallanScraper returned empty menu from fixture");
